var endlessRaiderApp = angular.module('endlessRaiderApp', [ 'ngRoute',
		'ngCookies', 'ngAnimate', 'xeditable', 'angularFileUpload',
		'textAngular', 'endlessRaiderController', 'agendaServices',
		'eventServices', 'authServices', 'userServices', 'persoServices',
		'jeuServices' ]);
...
endlessRaiderApp.service('Session', function($cookieStore) {
	this.create = function(sessionId, userId, userRole) {
		this.id = sessionId;
		this.userId = userId;
		this.userRole = userRole;

		$cookieStore.put('userSessionId', sessionId);
		$cookieStore.put('userSessionLogin', userId);
		$cookieStore.put('userSessionRole', userRole);
	};
	this.destroy = function() {
		...
		$cookieStore.put('userSessionId', "");
		$cookieStore.put('userSessionLogin', "");
		$cookieStore.put('userSessionRole', "");
	};
	this.get = function() {
		this.id = $cookieStore.get('userSessionId');
		this.userId = $cookieStore.get('userSessionLogin');
		this.userRole = $cookieStore.get('userSessionRole');
	};
	return this;
});

// Fonction qui permet de catcher les changements d'url afin d'appliquer les
// droits d'affichage
endlessRaiderApp.run(function($rootScope, AUTH_EVENTS, AuthService, $location,
		editableOptions) {
		...
		.config([
				...
				function($routeProvider, USER_ROLES) {
					$routeProvider
							...
							.when(
									'/administrer/editEvent/date/:jourEvent/:moisEvent/:anneeEvent',
									{
										templateUrl : 'front/pages/editEvent.html',
										controller : 'EvenementEditCtrl',
										data : {
							...
							.when(
									'/administrer/editEvent/template/:idTemplate',
									{
										templateUrl : 'front/pages/editEvent.html',
										controller : 'EvenementEditCtrl',
										data : {
							...
							.when(
									'/administrer/editTemplate/:idTemplate',
									{
										templateUrl : 'front/pages/editTemplate.html',
										controller : 'TemplateEditCtrl',
										data : {
